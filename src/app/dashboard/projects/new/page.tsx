  ...
  const onSubmit = handleSubmit(async (data) => {
    if (!params.projectId) {
      ...
      if (res.status === 201) {
        ...
        // una vez que hace la redireccion de arriba, le pido que refresque la pagina para actualizar lo que haya en cache
        router.refresh();
      }
    } else {
  ...
  useEffect(() => {
    if (params.projectId) {
      axios.get(`/api/projects/${params.projectId}`).then((res) => {
        // setValue es una funcion que me provee hook form para rellenar el form
